    ...
    def inference(self,
        ...
        for datum in tqdm(dataloader, disable=not self.is_main_process):
            ...
            with torch.no_grad():
                ...
                results, total_token_lens, input_token_lens = self.model.generate_from_template( ##########
                    entry, max_out_len=self.max_out_len, **extra_gen_kwargs)
                # results = self.model.generate_from_template( ##########
                #     entry, max_out_len=self.max_out_len, **extra_gen_kwargs)
                # results, reasoning_contents, total_token_lens, input_token_lens = self.model.generate_from_template( ##########
                #     entry, max_out_len=self.max_out_len, **extra_gen_kwargs)
                generated = results
            ...
            #         golds):
            for prompt, prediction, gold, total_token_len, input_token_len in zip( ##########
                    parsed_entries, batched(generated, num_return_sequences),
                    golds, total_token_lens, input_token_lens):
            # for prompt, prediction, gold, reasoning_content, total_token_len, input_token_len in zip( ##########
            #         parsed_entries, batched(generated, num_return_sequences),
            #         golds, reasoning_contents, total_token_lens, input_token_lens):
                if num_return_sequences == 1:
                    prediction = prediction[0]
                output_handler.save_results(prompt,
                                            prediction,
                                            index,
                                            gold=gold, total_token_len=total_token_len, input_token_len=input_token_len) ##########
                                            # reasoning_content=reasoning_content, total_token_len=total_token_len, input_token_len=input_token_len) ##########
                index = index + 1
