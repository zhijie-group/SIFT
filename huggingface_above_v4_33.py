...
class HuggingFacewithChatTemplate(BaseModel):
    ...
    def _load_tokenizer(self, path: Optional[str], kwargs: dict, pad_token_id: Optional[int] = None):
        ...
        # add special tokens : "<|reserved_special_token_0|>","<|reserved_special_token_1|>"
        # self.tokenizer.add_special_tokens({'additional_special_tokens': ["<|reserved_special_token_0|>", "<|reserved_special_token_1|>"]})
    ...
    def generate(self,
        ...
        # 计算除掉 padding token 之外的 token 数量
        input_token_len = tokens['input_ids'].shape[1] - (tokens['input_ids'] == self.tokenizer.pad_token_id).sum(dim=1) ##########
        input_token_len = input_token_len.tolist() ##########

        outputs = outputs[:, tokens['input_ids'].shape[1]:]
        output_token_len = (outputs != self.tokenizer.pad_token_id).sum(dim=1).tolist() ##########
        total_token_len = [input_token_len[i] + output_token_len[i] for i in range(len(input_token_len))] ##########
        ...
        return decodeds, total_token_len, input_token_len ##########
...
class HuggingFaceBaseModel(HuggingFacewithChatTemplate):
    ...
    def generate(self,
        ...
        # with torch.no_grad():
        #     for _ in range(10):
        #         _ = self.model.generate(**tokens, **generation_kwargs)
