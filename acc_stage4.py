...
parser.add_argument('--q_a2a', type=str, help='Path to the output JSONL file')
parser.add_argument('--a2p2', type=str, help='Path to the output JSONL file')
parser.add_argument('--q_a2p2', type=str, help='Path to the output JSONL file')
parser.add_argument('--p2a', type=str, help='Path to the output JSONL file')
parser.add_argument('--q_a2a2', type=str, help='Path to the output JSONL file')
parser.add_argument('--a2p3', type=str, help='Path to the output JSONL file')
parser.add_argument('--q_a2p3', type=str, help='Path to the output JSONL file')
parser.add_argument('--stage4res', type=str, help='Path to the output JSONL file')

q2p = parser.parse_args().q2p
q2a = parser.parse_args().q2a
a2p = parser.parse_args().a2p
q_a2p = parser.parse_args().q_a2p
q_a2a = parser.parse_args().q_a2a
a2p2 = parser.parse_args().a2p2
q_a2p2 = parser.parse_args().q_a2p2
p2a = parser.parse_args().p2a
q_a2a2 = parser.parse_args().q_a2a2
a2p3 = parser.parse_args().a2p3
q_a2p3 = parser.parse_args().q_a2p3
stage4res = parser.parse_args().stage4res
...
with open(a2p2, 'r') as f:
    gsm8k_final5 = [json.loads(line) for line in f]

with open(q_a2p2, 'r') as f:
    gsm8k_final6 = [json.loads(line) for line in f]

with open(q_a2a, 'r') as f:
    gsm8k_final7 = [json.loads(line) for line in f]



with open(a2p3, 'r') as f:
    gsm8k_final8 = [json.loads(line) for line in f]

with open(q_a2p3, 'r') as f:
    gsm8k_final9 = [json.loads(line) for line in f]

with open(q_a2a2, 'r') as f:
    gsm8k_final10 = [json.loads(line) for line in f]

with open(p2a, 'r') as f:
    gsm8k_final11 = [json.loads(line) for line in f]
...
for i in range(len(gsm8k_final1)):
    ...
    if evaluator_v2.is_equiv(gsm8k_final2[i]['pred_v2'], gsm8k_final3[i]['pred_v2']):
        ...
        flag = False
        gsm8k_final1_question = gsm8k_final1[i]['question'].split("\nPlease reason step by step, and put your final answer within")[0]
        for j in range(len(gsm8k_final5)):
            if gsm8k_final1_question in gsm8k_final6[j]['question']:

                total_token_len += gsm8k_final5[j]['total_token_len']
                total_token_len += gsm8k_final6[j]['total_token_len']
                total_token_len += gsm8k_final7[j]['total_token_len']

                if evaluator_v2.is_equiv(gsm8k_final5[j]['pred_v2'], gsm8k_final6[j]['pred_v2']):
                    flag = True
                    is_correct += gsm8k_final5[j]['is_correct_v2']
                    break

        if not flag:
            flag2 = False
            for j in range(len(gsm8k_final8)):
                if gsm8k_final1_question in gsm8k_final9[j]['question']:

                    total_token_len += gsm8k_final8[j]['total_token_len']
                    total_token_len += gsm8k_final9[j]['total_token_len']
                    total_token_len += gsm8k_final10[j]['total_token_len']
                    total_token_len += gsm8k_final11[j]['total_token_len']

                    if evaluator_v2.is_equiv(gsm8k_final8[j]['pred_v2'], gsm8k_final9[j]['pred_v2']):
                        flag2 = True
                        is_correct += gsm8k_final8[j]['is_correct_v2']
                        break
